...
function buildTicketOfferScene({draft,format,brand,logoUrl,sourceUrl},fallback=false) {
  ...
  const image=(id,src,x,y,width,height,role)=>{if(src)elements.push({id,name:id,role:role || id,type:'image',src,x,y,width,height,fit:'contain',keepRatio:true,locked:true});};
  ...
  if(!validation.valid && !fallback) return buildTicketOfferScene({draft:{...draft,style:'promo-editorial'},format,brand,logoUrl,sourceUrl},true);
