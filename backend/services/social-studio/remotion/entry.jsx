...
export function Campaign({layers,spec,backgroundColor}) {
  ...
  return <AbsoluteFill style={{backgroundColor,overflow:'hidden'}}>
    {layers.map(layer=>{
      ...
      const edge=track.bounds.y+track.bounds.height*state.reveal;
      return <AbsoluteFill key={layer.id} style={{opacity:state.opacity,transformOrigin:`${track.originX}px ${track.originY}px`,transform:`translate(${state.x}px,${state.y}px) scale(${state.scale})`,filter:`brightness(${state.brightness+state.light})`,clipPath:state.reveal<1?`polygon(0 0,100% 0,100% ${edge}px,0 ${edge}px)`:undefined}}><Img src={layer.src} style={{width:'100%',height:'100%'}} /></AbsoluteFill>;
    })}
  </AbsoluteFill>;
}
