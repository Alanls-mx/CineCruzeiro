...
function buildSessionAutocorrectPlan({ movies = [], rooms = [], tickets = [], orders = [], filters = {}, turnaroundMinutes = 20, includeSales = false, now = Date.now() } = {}) {
  ...
  byRoom.forEach((roomEntries) => {
    ...
    ordered.filter((entry) => entry.inScope && !locked.includes(entry)).forEach((entry) => {
      ...
      while (collision) {
        start = collision.end + turnaroundMs;
  ...
  const plan = { changes, unresolved, turnaroundMinutes: margin, includeSales: Boolean(includeSales) };
  return { ...plan, hash: planHash(plan) };
}

module.exports = { buildSessionAutocorrectPlan, planHash, _test: { cinemaParts, overlaps } };
