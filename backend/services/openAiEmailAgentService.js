...
async function generateOpenAiCampaignDraft(input = {}, options = {}) {
  ...
  try {
    ...
    if (!response.ok) {
      const error = new Error(`OpenAI respondeu HTTP ${response.status}.`);
      error.code = "OPENAI_RESPONSE_ERROR";
      throw error;
    }
...
module.exports = { generateOpenAiCampaignDraft, _test: { catalogFacts, responseText, RESPONSE_SCHEMA } };
