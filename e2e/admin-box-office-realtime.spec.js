...
test("cliente móvel reconecta após rede temporariamente offline e recupera estado após refresh", async ({ page, context }) => {
  ...
  await expect(page.getByText("Poltronas sincronizadas em tempo real.")).toBeVisible({ timeout: 10000 });
  ...
  await expect(page.getByText("Poltronas sincronizadas em tempo real.")).toBeVisible({ timeout: 15000 });
  await page.evaluate(() => document.dispatchEvent(new Event("visibilitychange")));
  await expect(page.getByText("Poltronas sincronizadas em tempo real.")).toBeVisible();
  await page.reload();
  await expect(page.getByText("Poltronas sincronizadas em tempo real.")).toBeVisible({ timeout: 15000 });
  await expect(page.getByRole("button", { name: /^A2,/ })).toHaveAttribute("aria-pressed", "true");
});
