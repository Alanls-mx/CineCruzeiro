...
test('modelos adicionais usam preço cadastrado e preservam a marca',async()=>{
  for(const formatId of ['feed_portrait','square','story']) {
    for(const [templateId,selection,expected] of [
      ['ticket-offer',{movieId:'m',priceSelection:{mode:'ticket-type',ticketTypeId:'meia',sessionId:'s'}},'12,00'],
      ...
      if(templateId==='ticket-offer') assert.ok(elements.some(item=>item.id==='description' && item.text.includes('Meia')));
