...
test("bloqueia uma segunda transferência do ingresso dentro de 24 horas", () => {
  ...
  assert.equal(result.code, "TICKET_TRANSFER_DAILY_LIMIT_REACHED");
  assert.ok(result.retryAfter > 0);
});

test("permite nova transferência do mesmo ingresso após 24 horas", () => {
  const result = evaluateTicketTransfer([
    transfer({ transferredAt: new Date(NOW - (25 * 60 * 60 * 1000)).toISOString() })
