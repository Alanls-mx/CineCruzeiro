...
test('codificador entrega MP4, WebM e GIF reais e limita concorrência',{skip:!available},async()=>{
  const first=exportAnimation(scene,{format:'mp4',duration:5,quality:'preview'});
  const queued=exportAnimation(scene,{format:'mp4',duration:5,quality:'preview'});
  const mp4=await first;
  assert.equal(mp4.buffer.toString('ascii',4,8),'ftyp');
  assert.equal((await queued).buffer.toString('ascii',4,8),'ftyp');
  const webm=await exportAnimation(scene,{format:'webm',duration:5,quality:'preview'});
  assert.equal(webm.buffer.subarray(0,4).toString('hex'),'1a45dfa3');
  const gif=await exportAnimation(scene,{format:'gif',duration:5,loop:false,quality:'preview'});
...
test('cancelamento e excesso de texto liberam a trava de exportação',async()=>{
  ...
  await assert.rejects(()=>exportAnimation({...scene,elements:[{type:'text',text:'Palavra '.repeat(170)}]}),/texto demais/);
});
